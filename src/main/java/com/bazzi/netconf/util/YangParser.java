...
public class YangParser implements SchemaSourceProvider<YangTextSchemaSource> {
    ...
    private final Map<SourceIdentifier, YangTextSchemaSource> sources = new HashMap<>();
    private final SharedSchemaRepository repository = new SharedSchemaRepository("yang-context-resolver");
    private SchemaContext schemaContext;
//    private final InMemorySchemaSourceCache<ASTSchemaSource> cache = InMemorySchemaSourceCache.createSoftCache(repository, ASTSchemaSource.class);
...
//        cache.schemaSourceEncountered(ast);
    ...
    public void retrieveSchemas(NetconfSession session, Map<String, String> schemas)
...
//                e.printStackTrace();
    ...
    public void retrieveSchemas(String path) {
        ...
        for (File curFile : files) {
            try {
                byte[] yangData = Files.readAllBytes(curFile.toPath());

                String name = curFile.getName();
                String[] split = StringUtils.split(name.substring(0,name.lastIndexOf(".")), "@");
